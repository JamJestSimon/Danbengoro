...
func set_truths(new_truths: Array):
	...
	bullets.clear()
	for truth in truths:
		var inst_bullet = bullet.instantiate() as TextureRect
		var label = (inst_bullet.get_child(0) as Label)
		label.text = truth
...
func set_special_truth(value: String):
	if value != "":
		special_truth = value
		(bullets[selected_truth].get_child(0) as Label).text = special_truth
